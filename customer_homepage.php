...
<! -- HEADER-PHP
    ...
    ?>
                                ...
                                $rest_ID = $rest_Name = $rest_MTable =  null;
                                $dish_Img = $dish_Name =  $dish_Type = $dish_Desc = $dish_Price = null;
                                $msg = $msgErr = null;
                            if ($_SERVER["REQUEST_METHOD"] == "POST" && $msg == null){
                                    if(isset($_POST["orderBtn"])){
                                       
                                        
                                            if (!empty($_POST["resID"])){
                                                $rest_ID = test_input($_POST["resID"]);

                                            }
                                            if (!empty($_POST["resName"])){
                                                $rest_Name = test_input($_POST["resName"]);

                                            }
                                        
                                            if (!empty($_POST["resTable"])){
                                                $rest_MTable = test_input($_POST["resTable"]);
                                        ...
                                        $order_Status = "send_to_restaurant";
                                        $rest_OrderTable = " ".$rest_MTable."_restaurant_orders"." ";
                                        $cusName = $_SESSION["c_name"];
                                        $cusAdd = $_SESSION["c_add"];
                                        $custID = $_SESSION["c_id"];
                                        //echo $rest_ID,$rest_Name,$rest_MTable,$dish_Img,$dish_Name,$dish_Type, $dish_Desc,$dish_Price,$cusName,$cusAdd;
                                        
                                        if($con_status == 1 && $rest_ID !== null && $rest_Name !== null && $rest_MTable !== null && $dish_Img !== null && $dish_Name !== null && $dish_Type !== null && $dish_Desc !== null && $dish_Price !== null ){
                                            $sql = "INSERT INTO $rest_OrderTable (customer_name, customer_address,dish_name, dish_image_dir, dish_price,customer_order_id,customer_order_table_name) VALUES ( '$cusName', '$cusAdd', '$dish_Name','$dish_Img', '$dish_Price','$custID','$cusOrderTable');"; 
                                            if(mysqli_query($conn,$sql)){
                                                $sql1 = "INSERT INTO $cusOrderTable (dish_name, restaurant_name,dish_price, dish_image_dir, order_status) VALUES ( '$dish_Name', '$rest_Name', '$dish_Price','$dish_Img', '$order_Status');";
                                                
                                                    if(mysqli_query($conn,$sql1)){
                                                            $msg = "Your order Has been received by the restaurant";
                                                             $dish_Name = $dish_Img = $dish_Price = $dish_Type= $dish_Desc =  null;
                                                            $rest_ID = $rest_Name = $rest_MTable  =null;
                                                        ...
                                                        $msgErr = "Error: " . $sql1 . "<br>" . mysqli_error($conn);}
                                        ...
                                        $msgErr = "Could not connect to database";
                                            }
    ...
    <header class="site-header" role="banner">
        ...
        <div class="collapse navbar-collapse justify-content-left" id="hbNavbar">
            ...
            <li class="nav-item ">
                ...
                <li class="nav-item">
                <a class="nav-link" href="customer_order.php"><h4>View Orders</h4><span class="sr-only">(current)</span></a>
                
                </li>
                <?php include("db_connect.php");
                 $ordernotif= null;
                 if($con_status == 1){
                    $custTable = " ".$_SESSION["c_TName"]."_orders"." ";
                    $sql = "SELECT count(customer_order_id) AS total FROM $custTable;"; 
        ...
        <button class="navbar-toggler navbar-toggler-right" type="button" data-toggle="collapse" data-target="#sbnavbar" aria-controls="sbnavbar" aria-expanded="False" aria-label="Toggle navigation"><i class="fas fa-user"></i></button>
            ...
            <li class="nav-item">
                    ...
                    <button type="button" class="btn btn-secondary" >
                     <?php echo $_SESSION["c_name"]." ";?>Orders <span class="badge badge-light"><?php echo $orderNotif;?></span>
                    </button>
                <li class="nav-item">
                                            ...
                                            while($row = mysqli_fetch_assoc($result)){
                                                ...
                                                if (mysqli_num_rows($result1) > 0){
                                                    ...
                                                    while($row = mysqli_fetch_assoc($result1)){
                                                        ...
                                                        <input type="hidden" name="resTable" value="<?php echo $resTableName;?>">
                                                        
                                                        
                                                        <button class="btn btn-success" type="submit" name="orderBtn" value="orderBtn">Order</button>
                                                        </form></li></ul>
                                                        </div></div>
